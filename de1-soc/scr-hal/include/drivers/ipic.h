...
{
    // configure IPIC
    for (int i = 0; i < PLF_IPIC_IRQ_VEC_NUM; ++i) ipic_irq_reset(i);
}
...
static inline unsigned long ipic_irq_current_vector(void) { return read_csr(IPIC_CISV); }

static inline unsigned long ipic_irq_current_get(void) { return read_csr(IPIC_CICSR); }

static inline void ipic_irq_current_set(unsigned long val) { write_csr(IPIC_CICSR, val); }
...
static inline void ipic_eoi(void) { write_csr(IPIC_EOI, 0); }

static inline void ipic_irq_enable(unsigned irq_vec)
{
    write_csr(IPIC_IDX, irq_vec);
    const unsigned long state = (read_csr(IPIC_ICSR) & ~IPIC_IRQ_PENDING) | IPIC_IRQ_ENABLE;
...
{
    write_csr(IPIC_IDX, irq_vec);
    const unsigned long state = read_csr(IPIC_ICSR) & ~(IPIC_IRQ_ENABLE | IPIC_IRQ_PENDING);
    write_csr(IPIC_ICSR, state);
}
...
{
    write_csr(IPIC_IDX, irq_vec);
    const unsigned long state = read_csr(IPIC_ICSR) | IPIC_IRQ_CLEAR_PENDING;
    write_csr(IPIC_ICSR, state);
}

static inline unsigned long ipic_pending(void) { return read_csr(IPIC_IPR); }

static inline unsigned long ipic_inservice(void) { return read_csr(IPIC_ISVR); }

static inline unsigned long ipic_mmapping(void) { return read_csr(IPIC_IMAP); }

static inline unsigned long ipic_mier(void) { return read_csr(IPIC_IER); }

static inline void ipic_mier_set(unsigned long val) { write_csr(IPIC_IER, val); }

#if PLF_MMU_SUPPORT
static inline unsigned long ipic_smapping(void) { return read_csr(IPIC_SIMAP); }

static inline unsigned long ipic_sier(void) { return read_csr(IPIC_SIER); }

static inline void ipic_sier_set(unsigned long val) { write_csr(IPIC_SIER, val); }
